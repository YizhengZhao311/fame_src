...
public class RFChecker {
	...
	public boolean isAReducedFormPositive(AtomicConcept concept, List<Formula> formula_list) {
		...
		for (Formula formula : formula_list) {
			...
			} else if (fc.positive(concept, formula) == 1 && fc.negative(concept, formula) == 0
					&& (formula.equals(concept)
							|| (formula instanceof Or && formula.getSubFormulas().contains(concept)))) {
	...
	public boolean isAReducedFormNegative(AtomicConcept concept, List<Formula> formula_list) {
		...
		for (Formula formula : formula_list) {
			...
			} else if (fc.negative(concept, formula) == 1 && fc.positive(concept, formula) == 0
					&& (formula.equals(new Negation(concept))
							|| (formula instanceof Or && formula.getSubFormulas().contains(new Negation(concept))))) {
