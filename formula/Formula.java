...
public class Formula {
	...
	public void setSubFormulas(Integer number, Formula formula1, Formula formula2) {
		this.number = number;
		this.subformulas.add(formula1);
		this.subformulas.add(formula2);
	}
	...
	public boolean equals(Object object) {
		...
		} else if (this instanceof Inverse && object instanceof Inverse) {
			Inverse inv = (Inverse) object;
			return this.getSubFormulas().get(0).equals(inv.getSubFormulas().get(0));
		} else if (this instanceof GreaterThan && object instanceof GreaterThan) {
			GreaterThan gt = (GreaterThan) object;
			return this.getNumber() == gt.getNumber() && this.getSubFormulas().get(0).equals(gt.getSubFormulas().get(0))
					&& this.getSubFormulas().get(1).equals(gt.getSubFormulas().get(1));
		} else if (this instanceof LessThan && object instanceof LessThan) {
			LessThan lt = (LessThan) object;
			return this.getNumber() == lt.getNumber() && this.getSubFormulas().get(0).equals(lt.getSubFormulas().get(0))
					&& this.getSubFormulas().get(1).equals(lt.getSubFormulas().get(1));
	...
	public boolean inverseComplement(Formula formula) {
		...
		return (formula1 instanceof Inverse && formula1.getSubFormulas().get(0).equals(formula2))
				|| (formula2 instanceof Inverse && formula2.getSubFormulas().get(0).equals(formula1));
	}
	...
	public Formula clone() throws CloneNotSupportedException {
		...
		if (this == TopConcept.getInstance()) {
			...
			return new Inverse(this.getSubFormulas().get(0).clone());

		} else if (this instanceof GreaterThan) {
			return new GreaterThan(this.getNumber(), this.getSubFormulas().get(0).clone(),
					this.getSubFormulas().get(1).clone());

		} else if (this instanceof LessThan) {
			return new LessThan(this.getNumber(), this.getSubFormulas().get(0).clone(),
					this.getSubFormulas().get(1).clone());
