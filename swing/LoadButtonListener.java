	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				System.out.println("===================================================");
				formulaList = bc.toAxioms(
						pp.getCNF(pp.getSimplifiedForm(pp.getClauses(ct.OntologyConverter(ontology)))));
				roleList = ct.getRolesInSignature(ontology);
				conceptList = ct.getConceptsInSignature(ontology);
				Collections.sort(roleList);
				Collections.sort(conceptList);
			} catch (OWLOntologyCreationException | CloneNotSupportedException e2) {
